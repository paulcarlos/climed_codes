...
# import TC IDs
sid <- readRDS("0_data/tc_dataframe_isimip3a_2000-2021.rds")

# pathnames
floc <- "D:/isimip3a_tc/ncfiles/" # folder of raw NC files downloaded from ISIMIP repository, each NC file contain rasters per hour at the boundary of winds or rainfall
fsav <- "D:/rast_isimip3a_tc/tcid/" # folder to save new NC files containing TC variables on global boundary WGS84
fhrly <- "D:/isimip3a_tc/hourly/" # folder to save hourly TC rasters

# select only 1 row, variable and model
#var <- "wind"; mod <- "er11"; mod1 <- "Emmanuel & Rotunno 2011"; nm <- "_obsclim_historical_"; short_vnm <- "m s-1"; long_vnm <- "1-minute sustained wind speed"
#var <- "rain"; mod <- "er11"; mod1 <- "Emmanuel & Rotunno 2011"; nm <- "_obsclim_historical_"; short_vnm <- "mm"; long_vnm <- "total rainfall"
...
wras <- rast(ext(), crs="OGC:CRS84", resolution=0.083333)
wras[] <- 0

# split TC IDs into groups to allow batch processing
#nrow(sid)/4
grp <- split(1:nrow(sid),rep(1:4,each=ceiling(nrow(sid)/4)))

# loop 
tcdf <- sid[grp[[3]],] # CHANGE GROUPING NUMBER HERE 1, 2, 3, or 4

for (i in 1:nrow(tcdf)) {
  id <- tcdf$tcid[i]
  cat("\n",i,": ",id,"\n")
  #ras1 <- terra::rast(paste0(floc,var,"/",mod,nm,var,"_",id,".nc"))
  ...
  if (any(lon<(-160)) & any(lon>160)) { # some rasters cross beyond WGS84 bounds i.e. Pacific Ocean
    lon1 <- which(lon<0) # negative
    lon2 <- which(lon>0) # positive
  }
  
  hr <- as.vector(ncvar_get(n1,"time"))
  tref <- as.POSIXct(gsub("hours since ","",n1$var$time$units),tz="UTC")
  hrv <- tref+hours(hr)
  ar1 <- ncvar_get(n1,var)
    
  filnm <- paste0(fsav,id,"/",var,"_",mod,"_",id,".nc") # filename to save per TC
  
  if (!file.exists(filnm)) {
    
    if (!is.null(dim(ar1)) & length(dim(ar1))>1) { # CHECK IF NOT EMPTY
    
      if (any(lon<(-160)) & any(lon>160)) { # SPLIT MATRIX/RASTER FOR ACROSS PACIFIC OCEAN FOLLOWING WGS84
        
        for (j in 1:length(hrv)) { # PROCESSING IS BY HOUR
          tim1 <- hrv[j] 
          m1 <- ar1[,,j] 
          if (length(dim(m1))==2) {
          ...
          if (sum(m1,na.rm=TRUE)>0) { # HOUTLY MATRIX SHOULD CONTAIN AT LEAST 1 GRID WITH A VALUE
            ...
            fnm2 <- paste0(fnm1,var,"_",mod,"_",id,"_",substr(tim1,1,10),"_",sprintf("%02d",hour(tim1)),"HR.tif") # FILENAME FOR HOURLY TIF files
            ...
            if (!file.exists(fnm2)) {
              ...
              } else if (is.vector(neglon) & is.vector(poslon)) {   # SINGLE GRID FOR BOTH
        ...
        r3 <- terra::rast(fnm3)
        writeCDF(r3, filename=paste0(fsav,id,"/",var,"_",mod,"_",id,".nc"),  # COMPILE ALL HOURLY TIF INTO A SINGLE NC FILE
                 varname=var, longname=long_vnm, unit=short_vnm, atts=c(paste0("model=",mod1)),
                 overwrite=TRUE, prec="float", compression=5)
        rm(fnm3,r3);gc()
        
      } else { # NO CROSSING OVER WGS 84 CRS, MAJORITY OF RAW NC FILES
    ...
    } else if (length(lon)==1 | length(lat)==1 & sum(ar1,na.rm=TRUE)>0) { # POSSIBLE VECTOR OR MATRIX FOR SINGLE LATITUDE OR LONGITUDE
